...
unset E_IUSE
IUSE="altivec debug gconf nls gtk nsplugin static vanilla"

gcj_pkg_setup() {
	if is_crosscompile; then
		eerror "cross-compiling not supported"
		die
	fi

	if ! use gtk ; then
		ewarn "The 'gtk' use flag is not active. AWT support will"
		ewarn "not be included as a result.  AWT applications such"
		ewarn "as OpenOffice and Eclipse/SWT will fail as"
		ewarn "a result."
	elog
	fi
...
gcj_do_configure() {
	...
	# find ecj-jar
	local ecjjar=$(java-pkg_getjar eclipse-ecj-3.3 ecj.jar)

	if [ -z ${ecjjar} ] ; then
	  ecjjar="/usr/share/java/ecj1.jar"
	fi
	einfo "Using ECJ jar ${ecjjar}."

	# gnu java / classpath config
	confgcc="${confgcc} --with-java-home=${PREFIX}/jre \
		$(use_enable gtk java-awt gtk) \
...
do_cleanup() {
	...
	dosym ${BINPATH}/gcj /usr/bin/gcj
	dosym ${BINPATH}/gij /usr/bin/gij
}
