...
pkg_setup() {
	...
	if [[ ${MERGE_TYPE} != "binary" ]] && is-flagq "-g*" && ! is-flagq "-g*0" ; then
		...
		einfo "You need to have at least ${CHECKREQS_DISK_BUILD} of temporary build space available"
...
src_prepare() {
	...
	# Fix building with both Wayland & X11
	# https://bugs.webkit.org/show_bug.cgi?id=140241
	epatch "${FILESDIR}"/${PN}-2.4.8-multiple-definition.patch
